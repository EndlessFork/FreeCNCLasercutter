...
class Data(object):
    ...
    def __init__(self,**kwds):
        ...
        self.ndX = 2*self.X
        self.ndY = 2*self.Y
        self.nerr = self.R**2 - self.X**2 - self.Y **2 -1
        del self.__dict__['R']
...
def kernel(data):
    ...
    def step_X(data, negative):
        if negative:
            ...
            data.nerr = data.nerr + data.ndX -1
            data.dX -= 2
            data.ndX -= 2
    ...
    def step_Y(data, negative):
        if negative:
            ...
            data.nerr = data.nerr + data.ndY -1
            data.dY -= 2
            data.ndY -= 2
    ...
    if data.octant == 0:
        ...
        assert (data.nerr - 2*data.Y <= -data.X) == (data.nerr -data.dY -1 <= -data.X)
        assert (data.nerr - 2*data.Y) == (data.nerr -data.ndY)
        #~ if data.err - 2*data.Y - 1 <= -data.X:
        if data.nerr -data.ndY <= -data.X:
            step_X(data, True)
        step_Y(data,False)
        ...
        assert (data.nerr + 2*data.X > data.Y) == (data.nerr +1 + data.dX > data.Y)
        assert (data.nerr + 2*data.X) == (data.nerr + data.ndX)
        #~ if data.err + 2*data.X - 1 > data.Y:
        if data.nerr + data.ndX > data.Y:
            step_Y(data, False)
        step_X(data, True)
        ...
        assert (data.nerr + 2*data.Y > -data.X) == (data.nerr +1 + data.dY > -data.X)
        assert (data.nerr + 2*data.Y) == (data.nerr + data.ndY)
        #~ if data.err + 2*data.Y -1 > -data.X:
        if data.nerr + data.ndY> -data.X:
            step_X(data, True)
        ...
        assert (data.nerr + 2*data.Y <= data.X) == (data.err + data.dY <= data.X)
        assert (data.nerr + 2*data.Y) == (data.nerr + data.ndY)
        #~ if data.err + 2*data.Y -1 <= data.X:
        if data.nerr + data.ndY <= data.X:
            step_X(data, False)
        ...
        assert data.X <= 0
        assert data.Y <= 0
        assert (data.nerr - 2*data.X > -data.Y) == (data.err - data.dX -2 > -data.Y)
        assert (data.nerr - 2*data.X) == (data.nerr - data.ndX)
        #~ if data.err - 2*data.X -1 > -data.Y:
        if data.nerr - data.ndX > -data.Y:
            step_Y(data, True)
        ...
        assert (data.nerr - 2*data.Y  > data.X) == (data.err - data.dY -2  > data.X)
        assert (data.nerr - 2*data.Y) == (data.nerr - data.ndY)
        #~ if data.err - 2*data.Y -1  > data.X:
        if data.nerr - data.ndY  > data.X:
            step_X(data, False)
        ...
        # octant -7: always x-, sometimes y-
        assert (data.nerr + 2*data.X > -data.Y) == (data.err + data.dX > -data.Y)
        assert (data.nerr + 2*data.X) == (data.nerr + data.ndX)
        #~ if data.err + 2*data.X -1 > -data.Y:
        if data.nerr + data.ndX > -data.Y:
            step_Y(data, True)
    ...
    assert data.ndX == 2*data.X
    assert data.ndY == 2*data.Y
    assert data.nerr == data.err -1
